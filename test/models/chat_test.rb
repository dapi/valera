...
class ChatTest < ActiveSupport::TestCase
  ...
  test 'takeover_time_remaining returns nil when manager_active_until is nil' do
    @chat.update!(mode: :manager_mode, taken_by: @user, taken_at: Time.current, manager_active_until: nil)
  ...
  test 'takeover_time_remaining returns remaining seconds' do
    freeze_time do
      timeout_minutes = ApplicationConfig.manager_takeover_timeout_minutes
      @chat.update!(
        mode: :manager_mode,
        taken_by: @user,
      ...
      assert_in_delta timeout_minutes.minutes.to_i, remaining, 1
    end
  end
  ...
  test 'takeover_time_remaining returns 0 when expired' do
    @chat.update!(
      ...
      taken_by: @user,
      taken_at: 1.hour.ago,
      manager_active_until: 30.minutes.ago
    )
  ...
  test 'takeover_expired? returns true when expired' do
    @chat.update!(mode: :manager_mode, taken_by: @user, taken_at: 1.hour.ago, manager_active_until: 30.minutes.ago)
  ...
  test 'manager_active? returns false when timeout expired' do
    chat = chats(:one)
    user = users(:one)
    ...
    travel 2.minutes do
      # manager_mode? stays true (no side effects)
      assert chat.manager_mode?
      # but manager_active? returns false when timeout expired
      assert_not chat.manager_active?
      # Note: auto-release happens via ChatTakeoverTimeoutJob, not via predicate method
    end
  end
  ...
  test 'in_manager_mode scope returns only manager-controlled chats' do
    chat1 = chats(:one)
    chat2 = chats(:two)
    user = users(:one)
  ...
  test 'in_ai_mode scope returns only bot-controlled chats' do
    chat1 = chats(:one)
    chat2 = chats(:two)
    user = users(:one)
