...
**АРХИТЕКТУРНЫЕ РЕШЕНИЯ:** [Memory Bank](.claude/memory-bank.md)

Содержит **ключевые решения проекта** и принципы архитектуры:
...
**Начать работу:** [docs/README.md](docs/README.md) - основная документация проекта
...
1. **📚 Основная документация:** [docs/README.md](docs/README.md) - **старт здесь**
2. **🏗️ Архитектурные решения:** [.claude/memory-bank.md](.claude/memory-bank.md)
3. **⚙️ Технологический стек:** Ruby on Rails 8.1 + ruby_llm + PostgreSQL
4. **🔧 Основные команды:** `bin/dev`, `bin/rails test`, `bin/rubocop`
...
- Требования и спецификации → изучение docs/FLOW.md и requirements/
...
**Процесс работы:** [docs/FLOW.md](docs/FLOW.md)
...
- **🚨 КРИТИЧЕСКИ ВАЖНО:** При создании моделей **ВСЕГДА** используй `rails generate model` (создает модель + миграцию + тесты + фикстуры одновременно). **НИКОГДА** не создавай миграцию отдельно! См. [Memory Bank Решение 4](.claude/memory-bank.md#решение-4-rails-model-generation---одновременное-создание-модели-и-миграции)
...
- Требования к проекту из ./docs/requirements архивируются и удаляются ТОЬЛКО в том случае если на это дал прямое указание пользователь. В противнм случае они все
  лежат где лежали с нужным статусом (выполнен, отменет, отложен и тп)