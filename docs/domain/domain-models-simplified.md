# 📋 Domain Models - Упрощенная модель для Super Valera бота
...
Минимальная доменная модель для Telegram бота Super Valera, который консультирует по кузовному ремонту и передает заявки менеджерам.
...
Бот: "Какого размера вмятина? Краска повреждена?"
Клиент: "Небольшая, краска целая"
Бот: "Это PDR случай, 5-10к₽. Хотите записаться?"
```

### Сценарий 2: ДТП
```
Клиент: "Попал в ДТП, есть КАСКО"
Бот: "Понял. Помогу с оформлением. Какие повреждения?"
...
- Задавать уточняющие вопросы о повреждениях
